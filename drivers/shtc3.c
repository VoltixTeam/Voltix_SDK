...
static int shtc3_sleep(void) {
  ...
  /* Put device to sleep */
  if ((ret = voltix_i2c_write_atomic(DEVICE_ADDRESS, cmd_buf, sizeof(shtc3_cmd_t))) != 0)
    return ret;
  return 0;
}
...
int shtc3_id(uint16_t *id) {
  ...
  if ((ret = voltix_i2c_write(DEVICE_ADDRESS, cmd_buf, sizeof(shtc3_cmd_t))))
    return -1;
  ...
  while (1) {
    if ((ret = voltix_i2c_write(DEVICE_ADDRESS, cmd_buf, sizeof(shtc3_cmd_t))) == 0)
      break;
    if (--retries == 0)
      return -1;
  }

  if ((ret = voltix_i2c_read(rx_buf, 3, DEVICE_ADDRESS)))
    return ret;
...
int shtc3_read(shtc3_res_t *res) {
  ...
  if ((ret = voltix_sleep_ticks(30)) != 0)
    return -2;

  if ((ret = voltix_i2c_read(rx_buf, 6, DEVICE_ADDRESS)))
    return -3;
