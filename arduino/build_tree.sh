...
VOLTIX_SDK_ROOT=${SCRIPT_PATH}/../
EXTERNAL_PATH=${VOLTIX_SDK_ROOT}/external
...
# Cleanup
rm -rf ${SCRIPT_PATH}/cores/Voltix/external/*
mkdir -p ${SCRIPT_PATH}/cores/Voltix/external/

cp -r ${VOLTIX_SDK_ROOT}/core ${SCRIPT_PATH}/cores/Voltix/external/
cp -r ${VOLTIX_SDK_ROOT}/drivers ${SCRIPT_PATH}/cores/Voltix/external/
cp -r ${VOLTIX_SDK_ROOT}/linker.ld ${SCRIPT_PATH}/cores/Voltix/external/linker.ld

# Copy files, creating paths as necessary
for item in "${copy_paths[@]}"
do
	src_path="${EXTERNAL_PATH}/${item}"
	dst_path="${SCRIPT_PATH}/cores/Voltix/external/${item}"
