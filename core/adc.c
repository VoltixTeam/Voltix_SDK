...
/* Inverse gain lookup table, indexed by voltix_adc_gain_t */
static const float gain_lut[] = {6.0f, 5.0f, 4.0f, 3.0f, 2.0f, 1.0f, 1.0f / 2, 1.0f / 4};
/* Reference lookup table, indexed by enum voltix_adc_reference_t */
...
float voltix_adc_adc2vadc(int16_t adc, voltix_adc_cfg_t *cfg) {
  ...
  if (cfg->input_neg != VOLTIX_ADC_INPUT_NC) {
    res_fac = 1.0f / (1 << 11);
  } else {
    res_fac = 1.0f / (1 << 12);
  }
...
int16_t voltix_adc_read(voltix_adc_input_t in) {
  ...
  NRF_SAADC->CH[0].PSELN = VOLTIX_ADC_INPUT_NC;
  NRF_SAADC->RESULT.PTR = (uint32_t)&result;
  NRF_SAADC->RESULT.MAXCNT = 1;
  NRF_SAADC->OVERSAMPLE = VOLTIX_ADC_OVERSAMPLE_DISABLED;
...
voltix_rc_t voltix_adc_sample(int16_t *dst, voltix_adc_cfg_t *cfg) {
  ...
  if (cfg->oversampling != VOLTIX_ADC_OVERSAMPLE_DISABLED)
