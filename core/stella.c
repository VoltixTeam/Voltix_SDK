...
static voltix_stella_pkt_t _rx_pkt_buf __attribute__((section(".retained_bss")));
static voltix_stella_pkt_t _tx_pkt_buf __attribute__((section(".retained_bss")));
static voltix_stella_pkt_t *rx_buf_ptr __attribute__((section(".retained_bss")));
...
void voltix_stella_init() {
  ...
  voltix_stella_set_id(NRF_FICR->DEVICEADDR[0]);
...
static inline int wait_for_completion(voltix_stella_pkt_t *rx_pkt, voltix_stella_pkt_t *tx_pkt) {
  ...
  if (notification_value & EVT_TEARDOWN)
    return VOLTIX_ERR_TEARDOWN;

  if (notification_value == EVT_STELLA_CRCERR)
    return VOLTIX_ERR_STELLA_NOACK;
  ...
  if (rx_pkt->hdr.ack_id != tx_pkt->hdr.pkt_id)
    return VOLTIX_ERR_STELLA_INVALIDACK;
...
static inline voltix_rc_t _transceive(voltix_stella_pkt_t *rx_pkt, voltix_stella_pkt_t *tx_pkt) {
  unsigned long notification_value;
...
voltix_rc_t voltix_stella_transceive(uint8_t *rx_buf, size_t rx_size, void *tx_data, size_t tx_size) {
  if (tx_size > VOLTIX_STELLA_MAX_DATA)
    return VOLTIX_ERR_OVERFLOW;

  memcpy(_tx_pkt_buf.data, tx_data, tx_size);
  _tx_pkt_buf.len = sizeof(voltix_stella_pkt_header_t) + tx_size;
  ...
  voltix_rc_t rc = _transceive(&_rx_pkt_buf, &_tx_pkt_buf);

  if (rc != VOLTIX_SUCCESS)
    return rc;

  size_t payload_size = _rx_pkt_buf.len - sizeof(voltix_stella_pkt_header_t);
...
voltix_rc_t voltix_stella_send(void *tx_data, size_t tx_size) {
  return voltix_stella_transceive(NULL, 0, tx_data, tx_size);
}

voltix_rc_t voltix_stella_receive(uint8_t *rx_buf, size_t rx_size) {
  return voltix_stella_transceive(rx_buf, rx_size, NULL, 0);
}
