...
#include "voltix_nvm.h"
#include "runtime.h"
#include "voltix_thresholds.h"

#define UNUSED(X) ((void)(X))

/* VOLTIX_STACK_SIZE is defined and passed in via the Makefile */
#define USR_STACK_SIZE_WORDS (VOLTIX_STACK_SIZE / sizeof(uint32_t))
...
#else
  return voltix_gpio_wait_level(PIN_PWRGD_H, VOLTIX_GPIO_LEVEL_HIGH, VOLTIX_GPIO_IN_NOPULL);
#endif
}
...
static void sys_handle_suspend(void) {
  ...
  if (gpint_unregister(PIN_PWRGD_H) == VOLTIX_GPIO_ERR_OK)
    xTaskNotifyIndexed(usr_task_handle, 1, EVT_GPIO_BASE, eSetBits);
  gpint_register(PIN_PWRGD_H, VOLTIX_GPIO_LEVEL_HIGH, VOLTIX_GPIO_IN_NOPULL, threshold_callback);
  ...
  if ((NRF_P0->IN & (1 << PIN_PWRGD_L)) == 0) {
    ...
    gpint_register(PIN_PWRGD_L, VOLTIX_GPIO_LEVEL_LOW, VOLTIX_GPIO_IN_NOPULL, threshold_callback);
  }
