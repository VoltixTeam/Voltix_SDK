...
#define VOLTIX_RC_BASE 0x00000000
#define VOLTIX_RC_STELLA_BASE 0x01000000
#define VOLTIX_RC_I2C_BASE 0x02000000

/**
 * @defgroup voltix Voltix basics
 *  @{
 */
...
enum {
  ...
  VOLTIX_SUCCESS = -(VOLTIX_RC_BASE + 0),
  /** Operation failed with generic error. */
  VOLTIX_ERR_GENERIC = -(VOLTIX_RC_BASE + 1),
  /** One or more invalid function arguments. */
  VOLTIX_ERR_INVALIDARG = -(VOLTIX_RC_BASE + 2),
  /** Operation was interrupted due to critically low capacitor voltage. */
  VOLTIX_ERR_TEARDOWN = -(VOLTIX_RC_BASE + 3),
  /** A reset occured during the operation. Most likely due to low capacitor voltage. */
  VOLTIX_ERR_RESET = -(VOLTIX_RC_BASE + 4),
  /** The requested operation would overflow a buffer. */
  VOLTIX_ERR_OVERFLOW = -(VOLTIX_RC_BASE + 5)
};

/** Common return code for Voltix SDK API functions. */
typedef int voltix_rc_t;
 ...
 *
 * @retval VOLTIX_SUCCESS       Capacitor voltage is above the 'high' threshold.
 * @retval VOLTIX_ERR_RESET     Reset occured while waiting.
 */
voltix_rc_t voltix_wait_cap_charged(void);
