#include <stdlib.h>
#include "nrf.h"
#include "voltix_gpio.h"

#include "FreeRTOS.h"
#include "runtime.h"
#include "gpint.h"
#include "voltix_gpio.h"

static GPINT_CALLBACK registry[42] = {0};

static TaskHandle_t blocking_task;

void GPIOTE_IRQHandler(void) {
  if (NRF_GPIOTE->EVENTS_PORT == 1) {
    for (uint32_t i = 0; i < 42; i++) {
      GPINT_CALLBACK cb = registry[i];
      voltix_gpio_port_t *reg = voltix_gpio_get_port(i);
      int pin_idx = voltix_gpio_get_pin_idx(i);
      if ((cb != NULL) && (reg->LATCH & (1 << pin_idx))) {
        gpint_unregister(i);
        reg->LATCH |= (1 << pin_idx);
        NRF_GPIOTE->EVENTS_PORT = 0;
        cb(i);
        return;
      }
    }
  }
}

int gpint_register(unsigned int pin, voltix_gpio_level_t level, voltix_gpio_in_pull_t pull, GPINT_CALLBACK cb) {
  voltix_gpio_port_t *reg = voltix_gpio_get_port(pin);
  int pin_idx = voltix_gpio_get_pin_idx(pin);

  if (pin > 41)
    return VOLTIX_GPIO_ERR_UNSUPPORTED;

  if (registry[pin] != NULL)
    return VOLTIX_GPIO_ERR_BUSY;
  registry[pin] = cb;

  reg->PIN_CNF[pin_idx] = (GPIO_PIN_CNF_INPUT_Connect << GPIO_PIN_CNF_INPUT_Pos) | (pull << GPIO_PIN_CNF_PULL_Pos);
  reg->LATCH |= (1 << pin_idx);

  /* Order is important, see nrf52833 errata 210*/
  if (level == VOLTIX_GPIO_LEVEL_HIGH) {
    reg->PIN_CNF[pin_idx] |= (GPIO_PIN_CNF_SENSE_High << GPIO_PIN_CNF_SENSE_Pos);
  } else {
    reg->PIN_CNF[pin_idx] |= (GPIO_PIN_CNF_SENSE_Low << GPIO_PIN_CNF_SENSE_Pos);
  }
  return VOLTIX_GPIO_ERR_OK;
}

int gpint_unregister(unsigned int pin) {
  if (registry[pin] == NULL)
    return VOLTIX_GPIO_ERR_GENERIC;

  voltix_gpio_port_t *reg = voltix_gpio_get_port(pin);
  int pin_idx = voltix_gpio_get_pin_idx(pin);

  reg->PIN_CNF[pin_idx] &= ~GPIO_PIN_CNF_SENSE_Msk;

  registry[pin] = NULL;
  return VOLTIX_GPIO_ERR_OK;
}

void voltix_gpio_init(void) {
  NRF_P0->DETECTMODE = GPIO_DETECTMODE_DETECTMODE_LDETECT;
  NRF_P1->DETECTMODE = GPIO_DETECTMODE_DETECTMODE_LDETECT;
  NRF_GPIOTE->EVENTS_PORT = 0;

  NRF_GPIOTE->INTENSET = GPIOTE_INTENSET_PORT_Msk;
  NVIC_EnableIRQ(GPIOTE_IRQn);
  NVIC_SetPriority(GPIOTE_IRQn, 1);
}

static void wait_callback(unsigned int pin_no) {
  BaseType_t xHigherPriorityTaskWoken = pdFALSE;
  xTaskNotifyIndexedFromISR(blocking_task, 1, EVT_GPIO_BASE, eSetBits, &xHigherPriorityTaskWoken);
  portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
}

voltix_rc_t voltix_gpio_wait_level(unsigned int pin, voltix_gpio_level_t level, voltix_gpio_in_pull_t pull) {
  unsigned long notification_value;
  taskENTER_CRITICAL();
  blocking_task = xTaskGetCurrentTaskHandle();
  xTaskNotifyStateClearIndexed(blocking_task, 1);
  ulTaskNotifyValueClearIndexed(blocking_task, 1, 0xFFFFFFFF);

  gpint_register(pin, level, pull, wait_callback);
  taskEXIT_CRITICAL();
  xTaskNotifyWaitIndexed(1, 0x0, 0xFFFFFFFF, &notification_value, portMAX_DELAY);
  if (notification_value & EVT_RESET)
    return VOLTIX_ERR_RESET;
  if (notification_value == EVT_GPIO_BASE)
    return VOLTIX_SUCCESS;
  return VOLTIX_ERR_GENERIC;
}
