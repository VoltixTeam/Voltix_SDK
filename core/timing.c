...
void RTC0_IRQHandler(void) {
  ...
  if ((NRF_RTC0->INTENSET & RTC_INTENSET_COMPARE0_Msk) && (NRF_RTC0->EVENTS_COMPARE[0] == 1)) {
    ...
    xTaskNotifyIndexedFromISR(blocking_task, 1, EVT_RTC_BASE, eSetBits, &xHigherPriorityTaskWoken);
  }
...
voltix_rc_t voltix_sleep_ticks(unsigned int ticks) {
...
voltix_rc_t voltix_sleep_ms(unsigned int ms) {
  ...
  return voltix_sleep_ticks((ms * 33554UL) >> 10);
}
