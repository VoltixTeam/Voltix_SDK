...
int voltix_max20361_init() {
  ...
  if ((rc = voltix_i2c_write_atomic(I2C_DEVICE_ADDRESS, tx_buf, 2)) != 0)
    return rc;
  ...
  return voltix_i2c_write_atomic(I2C_DEVICE_ADDRESS, tx_buf, 2);
}
...
int voltix_max20361_id(uint8_t *dev_id) {
  ...
  if ((rc = voltix_i2c_write(I2C_DEVICE_ADDRESS, &tx_buf, 1)) != 0)
    return rc;

  return voltix_i2c_read(dev_id, 1, I2C_DEVICE_ADDRESS);
}
