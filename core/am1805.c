...
static int read_registers(uint8_t* dst, size_t n_data, uint8_t reg_addr) {
  int rc;
  if ((rc = voltix_i2c_write(AM1805_I2C_ADDR, &reg_addr, 1)) != 0)
    return rc;
  return voltix_i2c_read(dst, n_data, AM1805_I2C_ADDR);
}
...
static int write_registers(uint8_t reg_addr, uint8_t* data, size_t n_data) {
  ...
  return voltix_i2c_write(AM1805_I2C_ADDR, tx_buf, n_data + 1);
}

static int write_register(uint8_t reg_addr, uint8_t data) {
  tx_buf[0] = reg_addr;
  tx_buf[1] = data;
  return voltix_i2c_write(AM1805_I2C_ADDR, tx_buf, 2);
}
...
int voltix_am1805_init(void) {
  uint16_t id;
  int retries = N_RETRIES;
  do {
    retries--;
    if (voltix_am1805_get_id(&id) == 0)
      if (id == 0x0518) {
        return 0;
      }
    voltix_sleep_ms(RETRY_DELAY_MS);
...
int voltix_am1805_enable_trickle(void) {
  int rc;
...
int voltix_am1805_get_datetime(struct tm* t) {
  uint8_t rx_buf[7];
  memset(t, 0, sizeof(t));
...
int voltix_am1805_set_datetime(struct tm* t) {
  uint8_t time_buf[7];
...
int voltix_am1805_get_hundredths(unsigned int* hundredths) {
  int rc;
  uint8_t rx_buf;
...
int voltix_am1805_get_alarm(struct tm* t) {
  uint8_t rx_buf[6];
  memset(t, 0, sizeof(t));
...
int voltix_am1805_reset(void) {
  set_configuration_key(AM1805_CONFIG_KEY_RESET);
  return 0;
}

int voltix_am1805_set_alarm(struct tm* t_alarm) {
  int rc;
  uint8_t time_buf[5];
...
int voltix_am1805_clear_alarm() {
  int rc;
  uint8_t status;
  if ((rc = voltix_am1805_get_status(&status)) != 0)
    return rc;
  status &= ~AM1805_STATUS_ALM_MSK;
  return write_register(AM1805_STATUS_REG, status);
}

int voltix_am1805_get_status(uint8_t* dst) {
  return read_register(dst, AM1805_STATUS_REG);
}

int voltix_am1805_disable_power(void) {
  uint8_t readback;
