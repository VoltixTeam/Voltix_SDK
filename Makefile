...
CORE_DIR := $(VOLTIX_SDK_ROOT)/core
DRIVER_DIR := $(VOLTIX_SDK_ROOT)/drivers
RTOS_DIR := $(VOLTIX_SDK_ROOT)/external/freertos
NRFX_DIR := $(VOLTIX_SDK_ROOT)/external/nrfx
CMSIS_DIR := $(VOLTIX_SDK_ROOT)/external/CMSIS_5
TFLIGHT_DIR := $(VOLTIX_SDK_ROOT)/external/tflm-cmsis
LINKER_SCRIPT:= $(VOLTIX_SDK_ROOT)/linker.ld
NRF_DEV_NUM := 52833

VOLTIX_STACK_SIZE ?= 2048
...
OBJS += $(subst $(VOLTIX_SDK_ROOT)/,$(OUTPUT_DIR)/, $(addsuffix .o, $(SDK_SRC_FILES)))
...
CFLAGS += -DVOLTIX_STACK_SIZE=${VOLTIX_STACK_SIZE}
...
LDFLAGS += -Wl,--defsym=VOLTIX_RAM_RETAINED_SIZE=${VOLTIX_RAM_RETAINED_SIZE}
...
${OUTPUT_DIR}/%.c.o: ${VOLTIX_SDK_ROOT}/%.c
	@mkdir -p $(@D)
...
flash: ${OUTPUT_DIR}/build.hex
	@echo Flashing: $<
	voltix-probe program -d nrf52 -f $<
