...
The Voltix runtime allows executing user-code on battery-free devices built around the Voltix Module.
...
You may increase the memory area where retained variables are stored by defining `VOLTIX_RAM_RETAINED_SIZE` in your application's Makefile.
...
:::
The runtime only checkpoints the part of the retained memory area that is actually occupied by the variables and the stack so don't worry about reducing VOLTIX_RAM_RETAINED_SIZE beyond the default value of 8192B.
However you may find that you can fit more static/global variables into the retained memory when you reduce the (generous) default `VOLTIX_STACK_SIZE` in your application's Makefile.
For an example, take a look at the [dsp example's Makefile](https://github.com/VoltixTeam/Voltix_SDK/blob/main/examples/dsp/Makefile).
